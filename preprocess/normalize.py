...
def one_hot_encode(df, one_hot_features):
    ...
    for (count, cols_feature) in enumerate(cols_features):
        # Pop the current col, then add to the original position
        cols_df.insert(loc_insert + count, cols_df.pop(cols_df.index(cols_feature)))
